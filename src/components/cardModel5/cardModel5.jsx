...
async function baixarEbook(titulo, setToast) {
  ...
  try {
    ...
    setToast({
      type: "error",
      message:
        error.response?.data?.error || "Erro desconhecido ao baixar o eBook.",
    });
  }
}

export default function CardModel5({ titulo, descricao, capa, url }) {
  const [toast, setToast] = useState(null); // Estado para armazenar o tipo e mensagem do toast

  return (
    <div className="my-10 w-80 md:w-full h-fit md:h-96 flex flex-col md:flex-row justify-between items-center bg-cinza-200_neutro bg-opacity-10 p-4 gap-6 rounded-xl backdrop-filter backdrop-blur-lg backdrop-saturate-150">
      <Image
        src={capa}
        className="w-80 min- h-96 md:h-fit rounded-lg"
        alt="Capa de ebook"
      />
