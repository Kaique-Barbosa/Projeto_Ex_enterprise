...
import GerarContratoSuccess from "@/components/Modal/GerarContratoSuccess";
...
export default function GerarContratoForm({ initialData }) {
  ...
  const enviarDadosParaLocacao = (data) => {
    ...
    api
      .post("/pdf/gerar", dataLocador)
      .then((response) => {
        setUrlDownload(response.data.url.downloadUrl);
        dialogSuccessRef.current.open();
  ...
  return (
    <>
      ...
      <GerarContratoSuccess ref={dialogSuccessRef} urlDownload={urlDownload} />
    </>
  );
}
